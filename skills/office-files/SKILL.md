...
7. 把一次性脚本写入 `/opt/data/tmp`，并在脚本顶部声明 PEP 723 依赖；脚本文本包含弯引号等易被规范化的标点时使用 `\u201c`、`\u201d`、`\u2018`、`\u2019` 转义。
8. 使用明确的容器输入路径；只有目录或批量任务才使用 `Path.glob()`。
9. 修改已有文件时另存为新文件；输出路径已存在时直接失败，不覆盖已有文件。
10. 先使用 `PYTHONUTF8=1 uv run --no-project python -m py_compile /opt/data/tmp/<script>.py` 做语法预检，再使用 `PYTHONUTF8=1 uv run /opt/data/tmp/<script>.py` 运行脚本。
11. 重新打开输出文件，验证目标结果，并比较写入前后的相关结构；出现非预期内容丢失、关系变化或对象数量变化时失败。
12. 成功后报告输出文件的完整路径并清理一次性脚本及其 `__pycache__`；失败时保留脚本并报告真实错误。
