...
凭证：只读取 Hermes Dock 注入的 ARK_AGENT_PLAN_API_KEY。
...
ERROR_HINTS = {
    ...
    "10403": (
        "提示：Agent Plan Key 无效或未开通豆包搜索。"
        f"请先在 {AGENT_PLAN_HARNESS_URL} 配置 Harness，"
        f"再从 {AGENT_PLAN_API_KEY_URL} 获取 Key 并保存到 Hermes Dock。"
    ),
    "10406": (
...
def _get_api_key() -> Optional[str]:
    api_key = os.getenv("ARK_AGENT_PLAN_API_KEY")
...
def _print_api_error_hint(code: str, message: str = "") -> None:
    ...
    if lower == "invalid_api_key" or "10403" in code_str:
        print(
            "提示：Agent Plan Key 无效或尚未为 Harness 开通豆包搜索。\n"
            "请在 Hermes Dock 的“火山方舟 Agent Plan”供应商页更新密钥，"
            "然后应用配置。",
            file=sys.stderr,
        )
        return
    ...
    if hint:
        ...
        print("请检查 Hermes Dock 的 Agent Plan 供应商配置和服务额度。", file=sys.stderr)
...
def main():
    parser = argparse.ArgumentParser(
        ...
        "凭证由 Hermes Dock 的火山方舟 Agent Plan 供应商配置提供"
    )
    ...
    if not api_key:
        print(
            "Error: 未配置火山方舟 Agent Plan API 密钥。"
    ...
    try:
        ...
        if exc.response is not None:
            ...
            if status == 429:
                ...
                print(
                    "提示：Agent Plan API Key 无效或已失效。"
                    "请在 Hermes Dock 的供应商页更新密钥并应用配置。",
                    file=sys.stderr,
                )
            else:
